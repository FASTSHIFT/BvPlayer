...
void Adafruit_PCD8544::begin(uint8_t contrast, uint8_t bias) {
  if (isHardwareSPI()) {
    // Setup hardware SPI.
    PCD8544_SPI.begin();
    PCD8544_SPI.setClockDivider(PCD8544_SPI_CLOCK_DIV);
    PCD8544_SPI.setDataMode(SPI_MODE0);
    PCD8544_SPI.setBitOrder(MSBFIRST);
  }
  else {
    // Setup software SPI.
