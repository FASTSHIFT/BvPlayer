/***************************************************
  This is a library for the Adafruit 1.8" TFT_SPI display.
...
as well as Adafruit raw 1.8" TFT display
  ...
  These displays use TFT_SPI to communicate, 4 or 5 pins are required to
...
// Constructor when using software TFT_SPI.  All output pins are configurable.
Adafruit_ST7735::Adafruit_ST7735(int8_t cs, int8_t dc, int8_t sid, int8_t sclk, int8_t rst)
    : Adafruit_GFX(ST7735_TFTWIDTH_128, ST7735_TFTHEIGHT_64)
{
    _cs   = cs;
    _dc   = dc;
    _sid  = sid;
...
// Constructor when using hardware TFT_SPI.  Faster, but must use TFT_SPI pins
...
inline void Adafruit_ST7735::spiwrite(uint8_t c) {
    ...
    if (hwSPI) {
        ...
        SPCR = SPCRbackup;
#else
        ST7735_SPI.transfer(c);
#endif
    } else {

        // Fast TFT_SPI bitbang swiped from LPD8806 library
...
void Adafruit_ST7735::writecommand(uint8_t c) {
#if defined (SPI_HAS_TRANSACTION)
    if (hwSPI)    TFT_SPI.beginTransaction(mySPISettings);
#endif
    DC_LOW();
    CS_LOW();
    ...
    CS_HIGH();
#if defined (SPI_HAS_TRANSACTION)
    if (hwSPI)    TFT_SPI.endTransaction();
#endif
}
...
Rcmd2green128x64[] = {              // Init for 7735R, part 2 (mini 128x64)
    2,                        //  2 commands in list:
    ST7735_CASET, 4,          //  1: Column addr set, 4 args, no delay:
    0x00, 67U,             //     XSTART = 67
    0x00, 130U,             //     XEND = 130
...
void Adafruit_ST7735::commonInit(const uint8_t *cmdList) {
    ...
    if(hwSPI) { // Using hardware TFT_SPI
        ST7735_SPI.begin(SPISettings(72000000, MSBFIRST, SPI_MODE0));
#if defined (SPI_HAS_TRANSACTION)
        TFT_SPI.begin();
        ...
        SPCRbackup = SPCR;
        TFT_SPI.begin();
        TFT_SPI.setClockDivider(SPI_CLOCK_DIV4);
        ...
        TFT_SPI.begin();
        TFT_SPI.setClockDivider(21); //4MHz
        TFT_SPI.setDataMode(SPI_MODE0);
#endif
    } else {
...
void Adafruit_ST7735::initR(uint8_t options) {
    ...
    if(options == INITR_GREENTAB) {
        ...
        colstart = 26;
        rowstart = 1;
    } else if(options == INITR_MINI128x64) {
        _height = ST7735_TFTHEIGHT_64;
        _width = ST7735_TFTWIDTH_128;
        commandList(Rcmd2green128x64);
        colstart = 2;
        rowstart = 67;
    } else {
    ...
    if ((options == INITR_BLACKTAB) || (options == INITR_MINI160x80) || (options == INITR_MINI128x64)) {
...
void Adafruit_ST7735::setRotation(uint8_t m) {
    ...
    switch (rotation) {
    case 0:
        if ((tabcolor == INITR_BLACKTAB) || (tabcolor == INITR_MINI160x80) || (tabcolor == INITR_MINI128x64)) {
        ...
        if (tabcolor == INITR_144GREENTAB) {
            ...
            _height = ST7735_TFTHEIGHT_160;
            _width = ST7735_TFTWIDTH_80;
        } else if (tabcolor == INITR_MINI128x64)  {
            _height = ST7735_TFTHEIGHT_64;
            _width = ST7735_TFTWIDTH_128;
        } else {
