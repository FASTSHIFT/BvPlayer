...
DRESULT disk_read(unsigned char pdrv, unsigned char *buff, DWORD sector, UINT count) {
  if (flashFatFs == NULL) {
    return RES_NOTRDY;
  }
  else {
...
DRESULT disk_write(unsigned char pdrv, const unsigned char *buff, DWORD sector, UINT count) {
  if (flashFatFs == NULL) {
    return RES_NOTRDY;
  }
  else {
